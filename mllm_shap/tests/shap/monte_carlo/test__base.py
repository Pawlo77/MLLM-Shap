...
class TestBaseMcShapExplainer:
    ...
    def test_get_next_split_without_minimal_masks(self, explainer: BaseMcShapExplainer) -> None:
    ...
    def test_calculate_shap_values_preserves_device_and_dtype(self, explainer: BaseMcShapExplainer) -> None:
        ...
        similarities = torch.tensor([0.5, 0.75, 1.25], dtype=torch.float64, device=device)
        result = explainer._calculate_shap_values(masks=masks, similarities=similarities, device=device)
        assert result.dtype == torch.float64
        assert result.device == device

    def test_calculate_shap_values_zero_difference(self, explainer: BaseMcShapExplainer) -> None:
