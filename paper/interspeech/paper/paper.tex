...
\begin{description}
  ...
  \item[Datasets] We evaluate our approach using two single-sentence English corpora derived from industry-standard benchmarks: (i)~\texttt{voice\_bench\_single\_sentence\_1k}, adapted from \textbf{VoiceBench}~\cite{chen2024voicebench}, which features paired synthetic (male and female) audio alongside original speech; and (ii)~\texttt{single\_sentence\_500}, adapted from \textbf{LibriSpeech-ASR}~\cite{panayotov2015librispeech}, which consists entirely of natural human speech. By leveraging these two datasets, we are able to analyze our contribution comprehensively across both artificial and authentic speech domains. Both curated corpora are publicly released alongside this work\footnote{\url{https://huggingface.co/datasets/Pawlo77/mllm-shap}}. \textbf{Due to hardware limitations, we limit our evaluation to the 100 prompts from each dataset.} Full details regarding the construction pipeline---including deduplication, sentence filtering, semantic quality filtering, token-budget filtering, and sampling---are provided in Appendix~\ref{app:dataset_creation}. \penalty-10000
