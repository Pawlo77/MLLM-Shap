...
from .helpers import as_list, rank_abs_sv, response_drop
from .io import parse_sample_id
from .models import FaithfulnessResult, RankwiseDeletionResult
from .sampling import quantile_bins, sample_random_set_matching_targets
...
class _SampleContext:
    """Shared pre-computed state for a single faithfulness sample."""
...
def _prepare_sample(
    ...
    """Load audio, align segments, and aggregate SVs for one sample."""
    ...
    return _SampleContext(
        ...
        segments=segments,
        segment_sv_values=segment_sv_values,
        sv_arr=sv_arr,
        intervals=intervals,
    )
...
def run_one_sample(
    ...
    def _mean_random_drops(
        ...
        For ``"stratified_random"`` baseline, also returns ``"strict_match_rate"``
        — the mean fraction of draws that achieved strict (duration+position) bin
        matching.  A value below 1.0 flags §5.1 bias-mitigation degradation.
        For ``"uniform_random"``, ``"strict_match_rate"`` is ``None``.
        """
        if draws <= 0:
            raise ValueError("random_draws must be positive.")
        specs: list[tuple[str, list[int]]] = []
        strict_match_rates: list[float] = []
        for _ in range(draws):
            rand_set, strict_rate = sample_random_set_matching_targets(
            ...
            if strict_rate is not None:
                strict_match_rates.append(strict_rate)
        ...
        out["strict_match_rate"] = (
            float(np.mean(strict_match_rates)) if strict_match_rates else None
        )
        return out
    ...
    def _mono_stats(
        ...
        # score = -rho: positive means higher-ranked segments cause larger drops
        # when deleted individually, i.e. the SV ordering is faithfully monotone.
    ...
    seq_neg_sim = float(neg_metrics["seqmatch"][1][0])
    seqmatch_neg_drop = response_drop(seq_orig, seq_neg_sim)
    seqmatch_neg_mean_random_drop = float(neg_uniform_random["seqmatch"])
    seqmatch_neg_drop_improvement = float(
        seqmatch_neg_mean_random_drop - seqmatch_neg_drop
    )

    seq_comp_sim = float(comp_metrics["seqmatch"][1][0])
    seqmatch_comp_drop = response_drop(seq_orig, seq_comp_sim)
    seqmatch_comp_mean_random_drop = float(comp_uniform_random["seqmatch"])
    seqmatch_comp_drop_difference = float(
        seqmatch_comp_drop - seqmatch_comp_mean_random_drop
    )

    seq_suff_sim = float(suff_metrics["seqmatch"][1][0])
    seqmatch_suff_drop = response_drop(seq_orig, seq_suff_sim)
    seqmatch_suff_mean_random_drop = float(suff_uniform_random["seqmatch"])
    seqmatch_suff_adv = float(seqmatch_suff_mean_random_drop - seqmatch_suff_drop)
    ...
    mono_seqmatch = _mono_stats(
        mono_metrics["seqmatch"][0], mono_metrics["seqmatch"][1]
    )

    pos_stratified_strict_match_rate = pos_stratified_random.get("strict_match_rate")
    neg_stratified_strict_match_rate = neg_stratified_random.get("strict_match_rate")
    comp_stratified_strict_match_rate = comp_stratified_random.get("strict_match_rate")
    suff_stratified_strict_match_rate = suff_stratified_random.get("strict_match_rate")
    ...
    return FaithfulnessResult(
        ...
        seqmatch_neg_sim=seq_neg_sim,
        seqmatch_neg_drop=seqmatch_neg_drop,
        seqmatch_neg_mean_random_drop=seqmatch_neg_mean_random_drop,
        seqmatch_neg_drop_improvement=seqmatch_neg_drop_improvement,
        seqmatch_comprehensiveness_sim=seq_comp_sim,
        seqmatch_comprehensiveness_drop=seqmatch_comp_drop,
        seqmatch_comprehensiveness_mean_random_drop=seqmatch_comp_mean_random_drop,
        seqmatch_comprehensiveness_drop_difference=seqmatch_comp_drop_difference,
        seqmatch_sufficiency_sim=seq_suff_sim,
        seqmatch_sufficiency_drop=seqmatch_suff_drop,
        seqmatch_sufficiency_mean_random_drop=seqmatch_suff_mean_random_drop,
        seqmatch_sufficiency_advantage=seqmatch_suff_adv,
        seqmatch_monotonicity_spearman=mono_seqmatch[0],
        seqmatch_monotonicity_score=mono_seqmatch[1],
        seqmatch_monotonicity_violation_rate=mono_seqmatch[2],
        pos_stratified_strict_match_rate=pos_stratified_strict_match_rate,
        neg_stratified_strict_match_rate=neg_stratified_strict_match_rate,
        comp_stratified_strict_match_rate=comp_stratified_strict_match_rate,
        suff_stratified_strict_match_rate=suff_stratified_strict_match_rate,
...
def run_one_sample_rankwise(
    ...
    sample_rate = ctx.sample_rate
    segments = ctx.segments
    segment_sv_values = ctx.segment_sv_values
    intervals = ctx.intervals
    ...
    seqmatch_orig, seqmatch_del_sims = sequence_match_similarities(
        model, base_resp, del_responses
    )
    ...
    for idx, (seg, del_sim, tfidf_del, seqmatch_del) in enumerate(
        ...
        results.append(
            RankwiseDeletionResult(
                ...
                seqmatch_deletion_drop=seqmatch_orig - seqmatch_del,
                seqmatch_cumulative_sim=seqmatch_cum,
                seqmatch_cumulative_drop=seqmatch_orig - seqmatch_cum,
